...
namespace SpiritMod.Items.Weapons
{
    public class TimeWinder : ModItem
    {
        ...
        public override void OnHitNPC(Player p, NPC target, int damage, float knockBack, bool crit)
        {
            if (Main.rand.Next(2) == 1)
            {
                ...
                int proj = Projectile.NewProjectile(p.Center.X, p.position.Y + p.height + -35, velocity.X, velocity.Y, mod.ProjectileType("TimeWinderClone"), damage = 60, item.owner, 0, 0f);
